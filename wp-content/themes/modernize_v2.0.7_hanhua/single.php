		...
		<?php 
			if ( have_posts() ){
				while (have_posts()){
					...
					comments_popup_link( __('发表评论','gdl_front_end'),
						__('1 条评论','gdl_front_end'),
						__('% 条评论','gdl_front_end'), '',
						__('评论已经关闭。','gdl_front_end') );
					echo '</div>';
					...
					echo the_content();
					wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( '页面:', 'gdl_front_end' ) . '</span>', 'after' => '</div>' ) );
					echo "</div>";
