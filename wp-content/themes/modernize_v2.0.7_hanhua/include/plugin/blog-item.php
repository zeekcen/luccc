	...
	*/
	...
	function print_blog_medium( $item_class, $item_size, $item_index, $num_excerpt ){
		...
		if( $gdl_admin_translator == 'enable' ){
			...
			$translator_continue_reading = __('阅读详情 →','gdl_front_end');
		}	
	...
	function print_blog_full( $item_class, $item_size, $item_index, $num_excerpt, $full_content = "No" ){
		...
		while( have_posts() ){
			...
			echo '<div class="blog-thumbnail-info post-info-color gdl-divider">';
			echo '<div class="blog-thumbnail-date">' . get_the_time( GDL_DATE_FORMAT ) . '</div>';
			echo '<div class="blog-thumbnail-author" style="display:none"> ' . __('','gdl_front_end') . ' ' . get_the_author_link() . '</div>';	
			the_tags('<div class="single-thumbnail-tag" style="display:none">', ', ', '</div>');
			
			echo '<div class="blog-thumbnail-comment" style="display:none">';
			comments_popup_link( __('发表评论','gdl_front_end'),
				__('1 条评论','gdl_front_end'),
				__('% 条评论','gdl_front_end'), '',
				__('评论已经关闭。','gdl_front_end') );
