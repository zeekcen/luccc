...
class flickr extends WP_Widget {
	...
	function flickr() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'flickr-widget', 'description' => __('显示最新 flickr 图像小工具。', 'gdl_back_office') );
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Flickr', 'gdl_back_office'), 'id' => __('0', 'gdl_back_office'), 'show_num' => '6', 'how' => 'lastest');
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_num' ); ?>"><?php _e('显示数量', 'gdl_back_office'); ?></label>
			...
			<label><?php _e('最多 "10"', 'gdl_back_office'); ?></label>
		</p>
		
        <p>
			<label for="<?php echo $this->get_field_id( 'how' ); ?>"><?php _e('图像显示类型','gdl_back_office'); ?></label><br />
			<select id="<?php echo $this->get_field_id( 'how' ); ?>" name="<?php echo $this->get_field_name( 'how' ); ?>" class="width100">
				<option <?php if ( 'latest' == $instance['how'] ) echo 'selected="selected"'; ?> value="latest">最新</option>            
				<option <?php if ( 'random' == $instance['how'] ) echo 'selected="selected"'; ?> value="random">随机</option>                  
			</select>
		</p>     
