...
class contact extends WP_Widget {
	...
	function contact() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'contact-widget', 'description' => __('联系表单小工具。', 'gdl_back_office') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'contact-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'contact-widget', __('联系表 (Goodlayers)', 'gdl_back_office'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( $gdl_admin_translator == 'enable' ){
			$translator_name = get_option(THEME_SHORT_NAME.'_translator_name_contact_widget', '姓名');
			$translator_email = get_option(THEME_SHORT_NAME.'_translator_email_contact_widget', '电子邮件');
			$translator_message = get_option(THEME_SHORT_NAME.'_translator_message_contact_widget', '信息');
			$translator_require = get_option(THEME_SHORT_NAME.'_translator_require_contact_widget', '* 必需');
			$translator_submit = get_option(THEME_SHORT_NAME.'_translator_submit_contact_widget', '提交');
			$translator_please_wait = get_option(THEME_SHORT_NAME.'_translator_please_wait_contact_widget', '稍等...');
			$translator_send_complete = get_option(THEME_SHORT_NAME.'_translator_sending_complete_contact_widget', '非常感谢！您的电子邮件已发送。');
		}else{
			$translator_name =  __('姓名','gdl_front_end');
			$translator_email = __('电子邮件','gdl_front_end');
			$translator_message = __('信息','gdl_front_end');
			$translator_require = __('* 必需','gdl_front_end');
			$translator_submit = __('提交','gdl_front_end');
			$translator_please_wait = __('稍等...','gdl_front_end');
			$translator_send_complete = __('非常感谢！您的电子邮件已发送。','gdl_front_end');
		}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('联络工具', 'gdl_back_office'), 'email' => '');
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>

		<!-- Widget Title: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('标题:', 'gdl_back_office'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e('电子邮件:', 'gdl_back_office'); ?></label>
