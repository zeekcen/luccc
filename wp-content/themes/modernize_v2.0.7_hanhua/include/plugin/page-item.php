	...
	*/
	...
	function print_price_item($item_xml){
		...
		if( $gdl_admin_translator == 'enable' ){
			$translator_read_more = get_option(THEME_SHORT_NAME.'_translator_read_more_price', '阅读更多');
		}else{
			$translator_read_more = __('阅读更多','gdl_front_end');
		}	
	...
	function print_contact_form($item_xml){
		...
		if( $gdl_admin_translator == 'enable' ){
			$gdl_name_string = get_option(THEME_SHORT_NAME.'_translator_name_contact_form', '姓名');
			$gdl_name_error_string = get_option(THEME_SHORT_NAME.'_translator_name_error_contact_form', 'Please enter your name');
			$gdl_email_string = get_option(THEME_SHORT_NAME.'_translator_email_contact_form', '电子邮件');
			...
			$gdl_message_string = get_option(THEME_SHORT_NAME.'_translator_message_contact_form', '信息');
			$gdl_message_error_string = get_option(THEME_SHORT_NAME.'_translator_message_error_contact_form', 'Please enter message');
			$gdl_submit_button = get_option(THEME_SHORT_NAME.'_translator_submit_contact_form','提交');
		}else{
			$gdl_name_string = __('姓名','gdl_front_end');
			$gdl_name_error_string =  __('Please enter your name','gdl_front_end');
			$gdl_email_string =  __('电子邮件','gdl_front_end');
			$gdl_email_error_string =  __('Please enter a valid email address','gdl_front_end');
			$gdl_message_string =  __('信息','gdl_front_end');
			$gdl_message_error_string = __('请输入信息','gdl_front_end');
			$gdl_submit_button = __('提交','gdl_front_end');
		}	
	...
	function print_portfolio($item_xml){
		...
		if( $gdl_admin_translator == 'enable' ){
			$translator_visit_website = get_option(THEME_SHORT_NAME.'_translator_visit_website', '访问网站');
			$translator_view_project = get_option(THEME_SHORT_NAME.'_translator_view_all_project', '查看所有项目 →');
			$translator_read_more = get_option(THEME_SHORT_NAME.'_translator_read_more', '阅读更多');
		}else{
			$translator_visit_website = __('访问网站','gdl_front_end');		
			$translator_view_project = __('查看所有项目 →','gdl_front_end');		
			$translator_read_more = __('阅读更多','gdl_front_end');
		}	
		...
		if( $filterable == "Yes" ){
			...
			foreach($category_lists as $category_list){
				...
				echo '<li><a href="#" class="gdl-button ' . $is_first . '" data-value="' . $category_slug . '">' . $category_list . '</a></li>';
