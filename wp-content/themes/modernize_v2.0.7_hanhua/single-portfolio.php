<?php get_header(); ?>
	<?php
		...
		if( $gdl_admin_translator == 'enable' ){
			...
			$translator_client =  __('客户','gdl_front_end');
			$translator_visit_website = __('访问网站','gdl_front_end');		
			$translator_about_author = __('关于作者','gdl_front_end');
			$translator_social_share = __('社交网络分享','gdl_front_end');
		}		
			...
			if ( have_posts() ){
				while (have_posts()){
					...
					if( get_option(THEME_SHORT_NAME.'_use_portfolio_as') == 'portfolio style' ){
						...
						echo '<div class="single-port-date"><span class="head">' . __('日期:','gdl_front_end') . '</span>';
						echo get_the_time( GDL_DATE_FORMAT ) . '</div>';
						
						$portfolio_tag = get_the_term_list( $post->ID, 'portfolio-tag', '', ', ' , '' );
						if(!empty($portfolio_tag)){
						echo '<div class="single-port-tag"><span class="head">' . __('标签:','gdl_front_end') . '</span>';
						echo $portfolio_tag;
						echo '</div>';
						}
						...
						echo '<div class="single-thumbnail-author" style="display:none"> ' . __('','gdl_front_end') . ' ' . get_the_author_link() . '</div>';
						$portfolio_tag = get_the_term_list( $post->ID, 'portfolio-tag', '', ', ' , '' );
						echo '<div class="single-thumbnail-tag" style="display:none">' . $portfolio_tag . '</div>';
						
						echo '<div class="single-thumbnail-comment" style="display:none">';
						comments_popup_link( __('发表评论','gdl_front_end'), __('1 条评论','gdl_front_end'),
							__('% 条评论','gdl_front_end'), '', __('评论已经关闭。','gdl_front_end') );
						echo '</div>';
						echo '<div class="clear"></div>';
						echo '</div>';
